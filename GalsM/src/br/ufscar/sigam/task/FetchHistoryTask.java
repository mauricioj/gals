...
import br.ufscar.sigam.converter.HistoricoConverter;
...
public class FetchHistoryTask extends AsyncTask<String, String, List<Object>> {
	...
	public FetchHistoryTask(TaskInterface<List<Object>> activity, User user, Curso curso, int id) {
	...
	private List<Object> doLogin() throws Exception {
		...
		String url = Config.URL_BASE + Config.getServiceListaHistorico(user.getUser(), user.getPass(), curso.getId());
		...
		if (response.getInt("status") == Constants.HTTP_OK) { 
			...
			for (int i = 0; i < array.length(); i++) {
				Historico c = HistoricoConverter.toObject(new JSONObject(array.getString(i)));
				contatos.add(c);
			}
		}
		return contatos;
